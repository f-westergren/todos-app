...
router.post('/actions', async (req, res, next) => {
    res.send('');
    const { trigger_id, user, actions, type, view, container } = JSON.parse(
        req.body.payload
    );
    ...
    if (actions && actions[0].action_id.match(/add-/)) {
    ...
    } else if (view.callback_id.match(/add-todo/)) {
        ...
        const creatingUser =
            selectedUsers.length > 0 ? selectedUsers.join(',') : user.id;
        ...
        view.callback_id.match(/mark-done/)
    ) {
